...
for row_index, row in enumerate(list_s2_1[0]):
    for col_index, col in enumerate(row):
        if(list_s2_1[7][row_index, col_index]>(longmin-360) and list_s2_1[7][row_index, col_index]<(longmax-360)):
            ...
            tempch3.append(list_s2_1[2][row_index, col_index])
            angles.append(col)
...
tempch3 = np.array(tempch3)[shared_indexes]
angles = np.array(angles)[shared_indexes]
lats = lats[shared_indexes]
longs = longs[shared_indexes]

trutharr=(tempch3>250)&(angles<60)
lats = lats[trutharr]
longs = longs[trutharr]
tempch3=tempch3[trutharr]

#plot channel 6 temperature vs location
plt.scatter(longs,lats,c=tempch3, cmap='viridis') 
plt.colorbar() 
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.title('Temperature in channel 3')
plt.show()
