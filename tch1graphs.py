...
tempch1=[]
angles=[]
for row_index, row in enumerate(list_s1_1[0]):
    for col_index, col in enumerate(row):
        if(list_s1_1[3][row_index, col_index]>(longmin-360) and list_s1_1[3][row_index, col_index]<(longmax-360)):
            longs.append(list_s1_1[3][row_index, col_index]+360)
            lats.append(list_s1_1[2][row_index, col_index])
            tempch1.append(list_s1_1[1][row_index, col_index])
            angles.append(col)
...
tempch1 = np.array(tempch1)[shared_indexes]
angles = np.array(angles)[shared_indexes]
lats = lats[shared_indexes]
longs = longs[shared_indexes]

trutharr=(tempch1<925)&(angles<60)
lats = lats[trutharr]
longs = longs[trutharr]
tempch1=tempch1[trutharr]

#plot channel 6 temperature vs location
plt.scatter(longs,lats,c=tempch1, cmap='viridis') 
plt.colorbar() 
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.title('Temperature in channel 1')
plt.show()
